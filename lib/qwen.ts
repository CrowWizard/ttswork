...
import { isSupportedAudioMimeType } from "@/lib/audio-format";
...
function isMockMode() {
  return (process.env.QWEN_MOCK_MODE ?? "true") === "true";
}
...
async function fetchTtsAudioFromPayload(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  payload: any,
): Promise<{ audioBuffer: Buffer; contentType: string; extension: string }> {
...
async function liveEnrollVoice(audioBuffer: Buffer, mimeType: string) {
  const url = assertEnv("QWEN_ENROLL_URL");
  const apiKey = assertEnv("QWEN_API_KEY");
...
async function liveSynthesizeSpeech(params: { text: string; voiceId: string }) {
  const url = assertEnv("QWEN_TTS_URL");
  const apiKey = assertEnv("QWEN_API_KEY");
  const requestPayload = {
...
export async function enrollVoice(params: { audioBuffer: Buffer; mimeType: string }) {
  if (isMockMode()) {
    return mockEnrollVoice(params.audioBuffer);
  }

  return liveEnrollVoice(params.audioBuffer, params.mimeType);
}

export async function synthesizeSpeech(params: { text: string; voiceId: string }) {
  if (isMockMode()) {
    return mockSynthesizeSpeech(params);
  }

  return liveSynthesizeSpeech(params);
}
