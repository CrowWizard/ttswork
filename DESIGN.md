...
### Header Modules
- **Style, typography, default/hover/active states, mobile treatment.** 当前不使用独立模块导航栏，模块入口收进全局 header，用于在“语音工作台”和“视频分析”之间切换。`register`、`settings` 等次级页面仍通过“个人设置”“返回工作台”“立即登录”等内联链接和按钮完成移动；这些入口必须保持任务语义，不扩展成营销导航或后台导航。
